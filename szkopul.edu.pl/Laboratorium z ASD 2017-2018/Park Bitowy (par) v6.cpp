...
Def. 5. Dla wierzchołka "u" drzewa, poddrzewo wierzchołka "u" to wierzchołek "u" oraz wszystkie wierzchołki drzewa, dla których wierzhołek "u" jest ich przodkiem
...
void inicjuj_struktury_danych() // dla każdego wierzchołka drzewa "u", określam wartości początkowe pewnych struktur danych
{
	REP(u,0,MAX_N-1) // dla każdego potencjalnego wierzchołka drzewa (wierzchołków może być nawet MAX_N)
	{
		...
		w[u] = -1; // skoro początkowe przybliżenie długości ścieżki ( związanej z wartością d[u] ) jest zerem, to ta ścieżka nie może kończyć się w żadnym wierzchołku drzewa (no bo trzeba by pójść co najmniej jedną krawędzią w górę, a potem (o ile to możliwe) gdzieś w dół drzewa - wynika to z tego, jakiego rodzaju ścieżki są opisane w tablicy "d" - patrz opis tablicy "d")
...
void f2(int u) // ta procedura, dla każdego wierzchołka "u" drzewa, oblicza tablice "d", "w" (pod warunkiem, że wywołamy ją dla korzenia, tzn. wywołamy ją dla argumentu "root")
{
	...
	REP(i,0,r-1) // dla każdego syna wierzchołka "u"
	{
		...
		if( d1[u].ND != v ) // jeśli przykładowy numer syna wierzchołka "u", przez którego przechodzi, dla wierzchołka "u", najdłuższa ścieżka I rodzaju, jest inny niż wierzchołek "v" - gdy tak rzeczywiście jest, to bierzemy do analizy najdłuższą, dla wierzchołka "u", ścieżkę I rodzaju (przypadek, gdzie "d1[u].ND" wynosi "-1", nie stwarza problemów)
		{
...
int zapytanie(int u, int odl) // dla wierzchołka "u" drzewa, znajduję wierzchołek odległy o "odl" od wierzchołka "u" (jeśli istnieje, to wypisuję jego numer (jak ich jest więcej - wypisuję numer któregokolwiek z nich), ale z zakresu [1..n]), bądź stwierdzam, że takiego w drzewie nie ma
{
	...
	// Uwaga. Warunek w poniższym "if-ie" nigdy się nie wykona, gdy u - korzeń drzewa (wartość "dmax_II" wynosi wtedy zero, więc żeby warunek tego "if-a" był spełniony, to musi być "odl" równe zero (bo zawsze zadajemy nieujemną wartość "odl") - skoro tak, to wiedząc, że wartość "dmax_I" jest zawsze nieujemna, to wykona się "if" z Przypadku 1 tej funkcji) - skoro tak, to unikniemy sytuacji, że będziemy mieć wartość "LCA" równą "-1", a potem bedziemy chcieli mieć wartość "depth[ LCA ]" ( odwołanie się do nieistniejącego indeksu w tablicy "depth" )
