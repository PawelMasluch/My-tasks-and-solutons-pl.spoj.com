...
int nr[MAX_N][MAX_K+1]; // dla każdego wierzchołka "u" drzewa, nr[u][j] oznacza numer wierzchołka, oddalonego od wierzchołka "u" o wartość 2^j (przy czym z wierzchołka "u" idziemy tylko w górę drzewa), lub "-1", gdy takowy wierzchołek nie istnieje - czyli, intuicyjnie, do jakiego wierzchołka dojdziemy, jeśli z wierzchołka "u" wykonamy w górę drzewa skok długości 2^j
...
void inicjuj_struktury_danych() // dla każdego wierzchołka drzewa "u", określam wartości początkowe pewnych struktur danych
{
	REP(u,0,MAX_N-1) // dla każdego potencjalnego wierzchołka drzewa (wierzchołków może być nawet MAX_N)
	{
		...
		{
			nr[u][j] = -1; // początkowo nie istnieje wierzchołek oddalony od wierzchołka "u" o "2^j"
		}
	}
}
