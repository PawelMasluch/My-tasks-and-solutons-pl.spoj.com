...
PII d2[MAX_N]; // dla każdego wierzchołka drzewa "u", d2[u] oznacza parę postaci (długość drugiej najdłuższej ścieżki w poddrzewie wierzchołka "u", wychodzącej z wierzchołka "u" (oczywiście ta ścieżka musi przechodzić przez innego syna wierzchołka "u", niż ścieżka najdłuższa) ; numer syna wierzchołka "u", przez którego ta ścieżka przechodzi)

// Uwaga (dla tablic d1, d2). Jeśli dla wierzchołka "u" są co najmniej 2 najdłuższe ścieżki, to jedna z nich jest opisana przez d1[u], natomiast druga - przez d2[u]   
...
//PII best[MAX_N]; // dla każdego wierzchołka drzewa "u", best[u] oznacza parę postaci (długość najdłuższej ścieżki wychodzącej z wierzchołka "u" ; numer wierzchołka kończącego tę ścieżkę) 

//int czy[MAX_N]; // dla każdego wierzchołka drzewa "u", czy[u] oznacza, czy najdłuższa ścieżka wychodząca z wierzchołka "u" to pojedyńcza gałąź (w przypadku zdegenerowanym, ścieżka pusta) idąca w dół drzewa (jeśli czy[u] = 0, to nie - wtedy mamy do czynienia z 2 sklejonymi w pewnym wierzchołku gałęziami; jeśli czy[u] = 1, to tak)
...
void inicjuj_struktury_danych() // dla każdego wierzchołka drzewa "u", określam wartości początkowe pewnych struktur danych
{
	REP(u,0,MAX_N-1)
	{
		d1[u] = d2[u] = MP( 0, -1 ); // początkowo zakładamy, że ścieżki, określone tablicami d1, d2, mają zerową długość (skoro tak, to nie przechodzą one przez żadnych synów)
		
		d[u] = MP( 0, -1 ); // początkowo zakładamy, że ścieżki, określone tablicami d1, d2, mają zerową długość (skoro tak, to nie przechodzą one przez żadnych synów)
...
/*void najdalsze() // dla każdego wierzchołka drzewa "u", znajduję najdłuższą ścieżkę, wychodzącą z wierzchołka "u", jak również określam typ tej ścieżki (patrz opis tablicy "czy")
{
	REP(u,0,n-1)
	{
		if( d1[u].ST >= d[u].ST )
...
int zapytanie(int u, int odl) // dla wierzchołka "u" drzewa, znajduję wierzchołek odległy o "odl" od wierzchołka "u" (jeśli istnieje, to wypisuję jego numer, ale z zakresu [1..n]), bądź stwierdzam, że takiego w drzewie nie ma
{
	...
	if( d_I >= odl ) // jeśli wierzchołek "v" (czyli wierzchołek najbardziej oddalony od wierzchołka "u") jest wystarczająco odległy od wierzchołka "u"
	{
		int v = w1[u];
		
		return znajdz( v, d_I - odl ) + 1;
	}
...
/*int zapytanie(int u, int odl) // dla wierzchołka "u" drzewa, znajduję wierzchołek odległy o "odl" od wierzchołka "u" (jeśli istnieje, to wypisuję jego numer, ale z zakresu [1..n]), bądź stwierdzam, że takiego w drzewie nie ma
{
	int v = best[u].ND, dmax = best[u].ST; // v - wierzchołek kończący najdłuższą ścieżkę, wychodzącą z wierzchołka "u";  dmax - długość najdłuższej ścieżki, wychodzącej z wierzchołka "u"
	...
	{
		return -1; // wypisujemy komunikat, że odpowiednio oddalony wierzchołek (tj. w odległości dokładnie "odl" od wierzchołka "u") od wierzchołka "u" nie istnieje		
	}
}*/
